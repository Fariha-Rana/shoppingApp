...
function Cart() {
  const { wishlistCount, cartCount } = useCount();
  const { userData } = useAuthentication();
  const router = useRouter()
  const userId = userData?.$id || false
...
const handleWishlistClick = async (e) => {
    ...
    if (!userId) {
      ...
      router.push("/wishlist/wishlistitem")
    }
  };

  return (
      <div className="flex justify-center items-center">
      <div className="relative py-2 mr-8">
